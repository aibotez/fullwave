...
function cline_angle_Callback(hObject, eventdata, handles)
...
tic
[x1,y1]=ray_tracing(x0(1),y0(1),kx0(1),ky0(1),I0(1),P0(1),beta_x0(1),beta_y0(1),dtau,step,R,Z,ne,IT,frequency,2);
R_judge=min(x1)+0.05;
[x,y,kx,ky,P,beta_x,beta_y]=ray_tracing(x0,y0,kx0,ky0,I0,P0,beta_x0,beta_y0,dtau,step,R,Z,ne,IT,frequency,R_judge);
toc
figure(2)
...
% plot(x(:,1),y(:,1))
 ...
 figure
 plot(y_rece,y_refl,'-s')
 hold on

ind1=find(y_rece>-0.3&y_rece<-0);
y_refl1=y_refl(ind1);
y_rece1=y_rece(ind1);
p=polyfit(y_rece1,y_refl1,1);
 R_cutoff=mean(R_cut)
 p(1)
%  plot(y_refl1,polyval(p,y_refl1),'r')
%  z_rece=[-0.1225 -0.1725];
 z_rece=[-0.064 -0.1247];
 
 z_refl=polyval(p,z_rece)
 dz=(z_refl(1)-z_refl(2))*100
 plot(z_rece,z_refl,'r')
 legend([num2str(frequency_1),'GHz'])
 %figure(4)
 %plot(y_rece,'-s')
 
function [x,y,kx,ky,P,beta_x,beta_y]=ray_tracing(x0,y0,kx0,ky0,I0,P0,beta_x0,beta_y0,dtau,cycle_length,R,Z,ne,IT,freq,R_judge)
...
while i<cycle_length
    ...
    [RK1_x,RK1_y,RK1_kx,RK1_ky]=RK4_Xmode(k0,kx1,ky1,P1,beta_x1,beta_y1,f,df_dx,df_dy,dg_dx,dg_dy);
     x1=x0+RK1_x*dtau/2;
     y1=y0+RK1_y*dtau/2;
     kx1=kx0+RK1_kx*dtau/2;
     ky1=ky0+RK1_ky*dtau/2;
     [f,df_dx,df_dy,dg_dx,dg_dy]=X_par(x1,y1,Dx,Dy,R,Z,ne,IT,freq);
     [RK2_x,RK2_y,RK2_kx,RK2_ky]=RK4_Xmode(k0,kx1,ky1,P1,beta_x1,beta_y1,f,df_dx,df_dy,dg_dx,dg_dy);
     x1=x0+RK2_x*dtau/2;
     y1=y0+RK2_y*dtau/2;
     kx1=kx0+RK2_kx*dtau/2;
     ky1=ky0+RK2_ky*dtau/2;
     [f,df_dx,df_dy,dg_dx,dg_dy]=X_par(x1,y1,Dx,Dy,R,Z,ne,IT,freq);
     [RK3_x,RK3_y,RK3_kx,RK3_ky]=RK4_Xmode(k0,kx1,ky1,P1,beta_x1,beta_y1,f,df_dx,df_dy,dg_dx,dg_dy);
     x1=x0+RK3_x*dtau;
     y1=y0+RK3_y*dtau;
     kx1=kx0+RK3_kx*dtau;
     ky1=ky0+RK3_ky*dtau;
     [f,df_dx,df_dy,dg_dx,dg_dy]=X_par(x1,y1,Dx,Dy,R,Z,ne,IT,freq);
     [RK4_x,RK4_y,RK4_kx,RK4_ky]=RK4_Xmode(k0,kx1,ky1,P1,beta_x1,beta_y1,f,df_dx,df_dy,dg_dx,dg_dy);
...
%input the density profile ne to acquire the 2D distribution of density
        ...
        psi1=reshape(psi(:,:,index),length(r),length(z));
        psi1_axis=psi_axis(index);
        psi1_bdy=psi_bdy(index);
