...
void savedata(double* R,double* Z,double** ez,double* receivedata1,double* receivedata2,double* t,double* phaset,double* phase1, double* phase2,double* ReflI, double* ReflQ)
{
	char strR[] = "R";
	...
	char strreceive1[] = "ReceiveData1";
	opt.writemat1d(receivedata1, strreceive1);
	char strreceive2[] = "ReceiveData2";
	opt.writemat1d(receivedata2, strreceive2);
	char strt[] = "t";
	opt.writemat1d(t, strt);
	char strtphase[] = "tphase";
	opt.writemat1d(phaset, strtphase);
	char strphase1[] = "phase1";
	opt.writemat1d(phase1, strphase1);
	char strphase2[] = "phase2";
	opt.writemat1d(phase2, strphase2);
	char strReflI[] = "ReflI";
...
int main()
{
	...
	int** ReceiveAntennaPoint1 = setAntenna(Ax1, 1.5 * AtL, w, 1.3 * AtL, ddx, ddy);
	int** ReceiveAntennaPoint2 = setAntenna(Ax2, 1.5 * AtL, w, 1.3 * AtL, ddx, ddy);
	//int** AntennaPoint = LaunchAntennaPoint;
	int** AntennaPoint1 = combinematrix(LaunchAntennaPoint, ReceiveAntennaPoint1);
	int** AntennaPoint = combinematrix(AntennaPoint1, ReceiveAntennaPoint2);
	...
	int nsteps = 60000;// 12000 * 3;//12000*3;//480000
	double* t = matht.CreatMatrix1d(nsteps,0);
	double sigtmp = 0;
	double UA = 5;
	double Uofft = 100;
	double Ui = 0;
	double Recesigtmp1 = 0;
	double Recesigtmp2 = 0;
	double* LaunchData = matht.CreatMatrix1d(nsteps, 0);
	double* ReceiveData1 = matht.CreatMatrix1d(nsteps, 0);
	double* ReceiveData2 = matht.CreatMatrix1d(nsteps, 0);
	double* SumData1 = matht.CreatMatrix1d(nsteps, 0);
	double* SumData2 = matht.CreatMatrix1d(nsteps, 0);


	double kxs[] = { 50,40,30,50,50,50,50,50,50 };
	double e = 1.602 * pow(10, -19);
	double me = 9.1096 * pow(10, -31);
	double epsilon0 = 8.85 * pow(10, -12);
	double ls;
...
#pragma omp parallel for
		for (int ii = 0; ii < nx; ii++)
		{
			...
			for (int jj = 0; jj < ny; jj++)
			{
				...
				ls = (ne[ii][jj] + nesm) * pow(e, 2) / epsilon0 / me;
				omega_pe[ii][jj] = ls;
	...
	//add source
		...
		idx1 = int((Ax1 - w / 1) / ddx);
		idx2 = (Ax1 + w / 1) / ddx;
		for (int i = idx1; i <= idx2; i++)
		{
			Recesigtmp1 = Recesigtmp1 + ez[i - 1][idx3 - 1];
		}
		sumtmp = idx2 - idx1 + 1;
		ReceiveData1[n - 1] = Recesigtmp1 / sumtmp;
		idx1 = int((Ax2 - w / 1) / ddx);
		idx2 = (Ax2 + w / 1) / ddx;
		for (int i = idx1; i <= idx2; i++)
		{
			Recesigtmp2 = Recesigtmp2 + ez[i - 1][idx3 - 1];
		}
		sumtmp = idx2 - idx1 + 1;

		ReceiveData2[n - 1] = Recesigtmp2 / sumtmp;
		SumData1[n - 1] = ReceiveData1[n - 1] + LaunchData[n - 1];
		SumData2[n - 1] = ReceiveData2[n - 1] + LaunchData[n - 1];
	...
	double* ReflI;
	double* ReflQ;
	cauphase(tphase,phase1,t, ReceiveData1,LaunchData, SumData1,f,ReflI,ReflQ);
	cauphase(tphase, phase2, t, ReceiveData2, LaunchData, SumData2, f, ReflI, ReflQ);
	...
	savedata(R,Z,ez, ReceiveData1,ReceiveData2,t,tphase,phase1, phase2,ReflI, ReflQ);
