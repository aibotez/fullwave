...
@cuda.jit
def GetReceiveData(ezG,idx1,idx2,idx1_2,idx3,Receives1,Receives2,n):
    i = cuda.blockDim.x * cuda.blockIdx.x + cuda.threadIdx.x
    j = cuda.blockDim.y * cuda.blockIdx.y + cuda.threadIdx.y
    if i < idx2-idx1+1:
        idxi = idx1 + i
        idxj = idx3
        Receives1[i,n-1] = ezG[idxi-1,idxj-1]
        idxi_2 = idx1_2 + i
        Receives2[i, n - 1] = ezG[idxi_2 - 1, idxj - 1]
...
class CauFullwave():
    def __init__(self):
        ...
        self.ReceiveDatas1 = None
        self.ReceiveDatas2 = None
        self.ReceiveData1 = None
        self.ReceiveData2 = None
        # self.LaunchDataG = None
        self.ReceiveDatas1G = None
        self.ReceiveDatas2G = None
        self.t = None
        ...
        self.ReflQ2 = None
        self.TurGs = None
        self.CurIterNums = None


    def _initparameters(self,f,nsteps,Turs,Vp):
        print('....../初始化参数/.......')
        self.CurIterNums = '....../初始化参数/.......'
        self.CurIterNums =0
        ...
        ReceiveAntennaPoint1 = self.setAntenna(self.Ax1, 1.5 * self.AtL,self.WGw, 1.3 * self.AtL, self.ddx, self.ddy)
        ReceiveAntennaPoint2 = self.setAntenna(self.Ax2, 1.5 * self.AtL, self.WGw, 1.3 * self.AtL, self.ddx, self.ddy)
        [nxLaunch,ignor] = LaunchAntennaPoint.shape
        [nxReceive1, ignor] = ReceiveAntennaPoint1.shape
        [nxReceive2, ignor] = ReceiveAntennaPoint2.shape
        self.AntennaPoints = np.zeros((nxLaunch+nxReceive1+nxReceive2,2)).astype(np.int32)
        self.AntennaPoints[0:nxLaunch,:] = LaunchAntennaPoint
        self.AntennaPoints[nxLaunch:nxLaunch+nxReceive1, :] = ReceiveAntennaPoint1
        self.AntennaPoints[nxLaunch+nxReceive1:nxLaunch+nxReceive1+nxReceive2, :] = ReceiveAntennaPoint2
        self.rowAn = int(nxLaunch+nxReceive1+nxReceive2)
    ...
    def cauphase(self):
        self.ReceiveDatas1 = self.ReceiveDatas1G.copy_to_host()
        self.ReceiveDatas2 = self.ReceiveDatas2G.copy_to_host()
        self.LaunchData = self.LaunchDataG.copy_to_host()
        [rowr,colr] = np.shape(self.ReceiveDatas1)
        y1_1 = np.sum(self.ReceiveDatas1,axis=0)/rowr
        y1_2 = np.sum(self.ReceiveDatas2, axis=0) / rowr
        self.ReceiveData1 = y1_1
        self.ReceiveData2 = y1_2
        y2 = self.LaunchData
        row = np.size(self.t)
        ...
        phase1 = np.zeros(row).astype(np.float64)
        ReflI1 = np.zeros(row).astype(np.float64)
        ReflQ1 = np.zeros(row).astype(np.float64)
        phase2 = np.zeros(row).astype(np.float64)
        ReflI2 = np.zeros(row).astype(np.float64)
        ReflQ2 = np.zeros(row).astype(np.float64)
        j=0
        i=0
        ysum_1 = y1_1+y2
        ysum_2 = y1_2 + y2
        ...
        y1_2 = np.abs(y1_2)
        y2 = np.abs(y2)
        ysum_1 = np.abs(ysum_1)
        ysum_2 = np.abs(ysum_2)
        while i < row-widx:
            I1_1 = math.pow(np.max(y1_1[i:i+widx]),2)
            I1_2 = math.pow(np.max(y1_2[i:i + widx]), 2)
            # I1 = np.square(np.max(y1[i:i+widx]))
            I2 = math.pow(np.max(y2[i:i + widx]), 2)
            # I2 = np.square(np.max(y2[i:i + widx]))
            Isum_1 = math.pow(np.max(ysum_1[i:i + widx]), 2)
            Isum_2 = math.pow(np.max(ysum_2[i:i + widx]), 2)
            # Isum = np.square(np.max(ysum[i:i + widx]))
            if I1_1 ==0:
                phase1[j] = 0
                # I1 = 1*10**(-10)
            else:
                cosph1 = (Isum_1 - I1_1 - I2) / (2 * math.pow(I1_1, 0.5) * math.pow(I2, 0.5))
                if cosph1 >1:
                    cosph1=1
                if cosph1 < -1:
                    cosph1 = -1
                phase1[j] = math.acos(cosph1)
                ReflI1[j] = I1_1 * math.cos(math.acos(cosph1))
                ReflQ1[j] = I1_1 * math.sin(math.acos(cosph1))
            if I1_2 ==0:
                phase2[j] = 0
                # I1 = 1*10**(-10)
            else:
                cosph2 = (Isum_2 - I1_2 - I2) / (2 * math.pow(I1_2, 0.5) * math.pow(I2, 0.5))
                if cosph2 >1:
                    cosph2=1
                if cosph2 < -1:
                    cosph2 = -1
                phase2[j] = math.acos(cosph2)
                ReflI2[j] = I1_2 * math.cos(math.acos(cosph2))
                ReflQ2[j] = I1_2 * math.sin(math.acos(cosph2))
        ...
        self.phase1 = phase1[0:j]
        self.phase2 = phase2[0:j]
        self.ReflI1 = ReflI1[0:j]
        self.ReflI2 = ReflI2[0:j]
        self.ReflQ1 = ReflQ1[0:j]
        self.ReflQ2 = ReflQ2[0:j]
    ...
    def SaveData(self):
        ...
        ReceiveData1 = self.ReceiveData1
        ReceiveData2 = self.ReceiveData2
        ReflI1 = self.ReflI1
        ReflQ1 = self.ReflQ1
        ReflI2 = self.ReflI2
        ReflQ2 = self.ReflQ2
    ...
    def Gpurun(self):
        ...
        idx11 = int((self.Ax1 - self.WGw / 1) / self.ddx)
        idx22 = int((self.Ax1 + self.WGw / 1) / self.ddx)
        idx11_2 = int((self.Ax2 - self.WGw / 1) / self.ddx)
        idx22_2 = int((self.Ax2 + self.WGw / 1) / self.ddx)
        self.ReceiveDatas1 = np.zeros((idx22-idx11+1,self.nsteps)).astype(np.float64)
        self.ReceiveDatas1G = cuda.to_device(self.ReceiveDatas1)
        self.ReceiveDatas2 = np.zeros((idx22_2-idx11_2+1,self.nsteps)).astype(np.float64)
        self.ReceiveDatas2G = cuda.to_device(self.ReceiveDatas2)
        ...
        for i in range(1,self.nsteps+1):
            ...
            cuda.synchronize()
            GetReceiveData[gridDim_addS,blockDim_addS](self.ezG,idx11, idx22, idx11_2,idx3,self.ReceiveDatas1G,self.ReceiveDatas2G,i)
            cuda.synchronize()
            ...
            # plt.imshow(dz0.T)
            # plt.show()
        self.CurIterNums='finsh'
        end = time.time()
        print('用时：',end-start)
        self.CurIterNums = 'finsh! 用时：' + str(end-start)
        self.cauphase()
        self.SaveData()
...
Test=CauFullwave()
Test._initparameters(f,nsteps,Turs,Vt)
Test.Gpurun()
