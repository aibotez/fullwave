...
@cuda.jit
def Dne(omega_pe,ne0,vt,ntp,dt,ddx,ddy,nx,ny,Turs,lambda0,TuryDis):
    ...
    if i < nx and j < ny:
        ...
        at = ne0[i, j] * 1 / 100
        for kxi in Turs:
            Atur = at * TuryDis[j]*math.sin(kxi * (xi - vt * t))
            nesm = nesm + Atur
...
class CauFullwave():
    ...
    def _initparameters(self,f,nsteps,Turs,Vp):
        ...
        self.GetCutoffPosition()
        self.InitTuryDis()
        # print(self.yCutoffPosition)
    ...
    def InitTuryDis(self):
        # TuryDis = np.zeros(self.ny).astype(np.float64)
        wf = self.lambda0
        dyw = np.square((self.Z - self.yCutoffPosition) / wf)
        self.TuryDis=np.exp(-dyw)
        self.TuryDisG = cuda.to_device(self.TuryDis)
    def GetCutoffPosition(self):
        e = 1.602*10**(-19)
        me = 9.1096*10**(-31)
        epsilon0 = 8.85 * 10**(-12)
        neCol = self.ne[1,:]
        omega_pe = np.sqrt(neCol*e**2/epsilon0 / me)
        omega0 = 2*np.pi*self.f
        idxs = np.where(omega_pe>=omega0)[0]
        if idxs[0] > idxs[-1]:
            idx = idxs[-1]
        else:
            idx = idxs[0]
        self.yCutoffPosition = self.Z[idx]
    ...
    def Gpurun(self):
        ...
        for i in range(1,self.nsteps+1):
            ...
            Dne[self.blocks_per_grid,self.threads_per_block](self.omega_peG,self.neG,Vt, i, self.dt, self.ddx, self.ddy, self.nx, self.ny,self.TursG,self.lambda0,self.TuryDisG)
            cuda.synchronize()
