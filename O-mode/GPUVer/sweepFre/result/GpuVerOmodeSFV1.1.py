...
@cuda.jit
def setAntennaBoundary(AntennaPoint,rowAn,dzG,yMidx):
    i = cuda.blockDim.x * cuda.blockIdx.x + cuda.threadIdx.x
    j = cuda.blockDim.y * cuda.blockIdx.y + cuda.threadIdx.y
    if i <rowAn:
...
def AddSource(dzG,lenaddS,idx0,idx3,ddx,xatp,f,dt,t0,T,w,Launchdata):
    ...
    if (i<lenaddS):
        ...
        dti = 10/f
        df = 0.1 * 10 ** 9
        ft = df * int(n * dt / dti) + f
        # ft = f*(int(n*dt/dti)/int(dt0/dti))+f
        # ft = f*n*dt/(30000*dt)+f
        # signal = Ui * math.exp(-pl ** 2 / (w ** 2)) * math.cos(-2 * np.pi * ft * dt * (t0 - T))

        B = 0.3 * f
        tm = dt0

        k = 2 * B / tm
        signal = Ui * math.exp(-pl ** 2 / (w ** 2)) * math.cos(2 * np.pi * f * dt * T+np.pi*k*(dt*T)**2)
        dzG[idxi-1,idxj-1] = dzG[idxi-1,idxj-1]+signal
        if i==0 and j==0:
            # Launchdata[T - 2] = dzG[idx0+int(lenaddS/2)-1,idx3-1]
            Launchdata[T-2] = Ui * math.cos(2 * np.pi * f * dt * T+np.pi*k*(dt*T)**2)
...
class CauFullwave():
    ...
    def _initparameters(self,f,nsteps,Turs,Vp):
        ...
        self.nsteps = nsteps
        self.ne = self.Caune(self.R,self.Z)*1
    ...
    def Gpurun(self):
        ...
        y0 = 0.25
        kyM = (0.4-y0)/self.nsteps

        yM = 0.45
        yMidx = int(yM / self.ddy)
        ...
        for i in range(1,self.nsteps+1):
            ...
            setAntennaBoundary[gridDim_setAnt,blockDim_setAnt](self.AntennaPointsG,self.rowAn, self.dzG,yMidx)
            cuda.synchronize()
