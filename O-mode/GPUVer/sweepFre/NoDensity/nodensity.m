...
[Anxr,Anyr]=setReceiverAntenna(Ax,1.5*AtL,w,1.3*AtL,ddx,ddy);
Anx=[Anxl Anxr];
Any=[Anyl Anyr];
% figure
% imagesc(x,y,ne');
...
%        sig = [sig signal];
   ...
   iget = fix((Ax-w/1)/ddx):fix((Ax+w/1)/ddx);
   Erec = [Erec mean(ez(iget,fix((yatp)/ddy)))];
...
sigr = Erec;
sign = sig0+sigr;
[tn,phase] = cauphase(t0,sig0,sigr,sign,f,fs);
figure
plot(tn,phase)
toc
